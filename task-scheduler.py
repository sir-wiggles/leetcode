...
class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        order = sorted(Counter(tasks).values(), reverse=True)
        max = order[0] - 1
        slots = max * n
        for v in order[1:]:
            slots -= min(v, max)
...
class Test(unittest.TestCase):
    ...
    def test5(self):
        ...
        n = 2
        self.assertEqual(Solution().leastInterval(tasks, n), 9)
    def test6(self):
    ...
    def test8(self):
        tasks = ["A","A"]
        n = 1
        self.assertEqual(Solution().leastInterval(tasks, n), 3)
    def test9(self):
        ...
        n = 1
        self.assertEqual(Solution().leastInterval(tasks, n), 1000)
